    ...
    return GetBuilder<FrontendController>(
      builder: (controller) {
        return FutureBuilder<Map<String, dynamic>?>(
          future: controller.partnerDiscountProducts,
          builder: (buildContext, asyncSnapshot) {
            if(asyncSnapshot.connectionState == ConnectionState.done) {
              if(asyncSnapshot.hasData) {
                final discountProducts = asyncSnapshot.data;
                List<PartnerProductModel> data = [];
                for (var i = 0; i < (discountProducts?["result"]?.length ?? 0); i++) {
                  PartnerProductModel model
                    = PartnerProductModel.fromJson(discountProducts?["result"][i]);
                  data.add(model);
                }
                ...
                : Column(
                  ...
                  children: [
                    ...
                    SectionTitle(
                      titleText: lController.getLang("Discounted Products"),
                      isReadMore: true,
                      onTap: () => Get.to(() => const PartnerProductsScreen(appTitle: "Discounted Products", dataFilter: { "statuses": [ 6 ] } )),
