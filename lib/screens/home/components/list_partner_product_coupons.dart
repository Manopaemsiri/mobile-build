...
import 'package:coffee2u/screens/partner/product_coupon/list.dart';
...
class ListPartnerProductCoupons extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<FrontendController>(builder: (controller) {
      return FutureBuilder<Map<String, dynamic>?>(
        future: controller.partnerProductCoupons,
        builder: (context, snapshot) {
          if(snapshot.connectionState == ConnectionState.done) {
            if (snapshot.hasData) {
              List<PartnerProductCouponModel> items = [];
              var len = snapshot.data!['result'].length;
              for (var i = 0; i < len; i++) {
                PartnerProductCouponModel model =
                  PartnerProductCouponModel.fromJson(snapshot.data!['result'][i]);
                items.add(model);
              }
              ...
              if (items.isEmpty) {
                ...
                return Column(
                  ...
                  children: [
                    ...
                    SectionTitle(
                      titleText: lController.getLang("Promotional Product Coupons"),
                      isReadMore: true,
                      onTap: () => Get.to(() => const PartnerProductCouponsScreen()),
  ...
  onTap(String id) =>
    Get.to(() => PartnerProductCouponScreen(id: id));
}
